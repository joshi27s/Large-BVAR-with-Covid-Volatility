...
lags = 12  # Number of lags in VAR, if monthly data
...
plotDir = 'Entropic Tilting Plots/'
os.makedirs(plotDir, exist_ok=True)
# Define the start date for forecasting
vint = datetime(2024, 11, 1)
...
# Remove 'Date' column and convert data to numpy array
...
# Federal Funds Rate
idxCVFFR = Spec[Spec['SeriesName'] == 'Federal Funds Rate'].index[0]
valCVFFR = (2.9 + 3.6) / 2
...
Tfeb2020 = np.where((dates.dt.year == 2020) & (dates.dt.month == 2))[0][0]
...
fore_dates = pd.date_range(start=pd.Timestamp(f'{yEnd}-{mEnd + 1}-01'), periods=h, freq='MS')
...
dPredY_unc = np.vstack((
    np.full((12, n, ndraws), np.nan),  # Add NaNs at the beginning
    PredY_unc[12:, :, :] - PredY_unc[:-12, :, :]
))

# MCMC Draws for tilting variables (PCE inflation, unemployment rate, and federal funds rate) at forecast horizon
YYh = np.vstack((
    dPredY_unc[-1, idxCVPCE , :].T,  # Extract indices of federal funds rate and PCE inflation rate
    PredY_unc[-1, [idxCVLR, idxCVFFR], :]                  # Extract unemployment rate
)).T
# Target values: FOMC's centeral tendency projections for the targeted variables
target = np.array([valCVPCE, valCVLR, valCVFFR])
...
if plot_uncondi_forecasts_tilted:
    ...
    for iVar in range(n):  # Looping over variables in Spec
        ...
        if Spec.loc[iVar, 'Transformation'] == 'log':
            ...
            ax.set_title(f"{Spec.loc[iVar, 'SeriesName']} (12-month percent change)")
        else:
...
if plot_joint_distribution:
    # SEP Forecast values
    U32026 = 4.25  # central tendencies of SEP Forecast of Unemployment rate for 2026: 4.1-4.4
    U32027 = 4.2  # central tendencies of SEP forecast for 2027: 4-4.4 percent
    PCE2026 = 2.1  # central tendencies of SEP Forecast of PCE inflation rate for 2026: 2-2.2
    PCE2027 = 2.0  # central tendencies SEP forecast for 2027: 2-2 percent
    ...
    YY = np.array([dPredY_unc[-1, idxCVPCE, :], PredY_unc[-1, idxCVLR, :]]).T
    fig, ax = bvar.plot_weighted_joint_and_marginals(YY, wStarMedian, 'PCE', 'U3', vis=True, LW=1.5, Y0=[PCE2027, U32027])
    ax.set_title('Forecast for Nov 2027 in Dec 2024')
    plt.savefig(f'{plotDir}/jointMarginal_PCE_LR_ET_202711.png', dpi=dots_per_inch)
    plt.close(fig)

    # Plot joint/marginal for Nov 2026 in Dec 2024
    YY = np.array([dPredY_unc[-13, idxCVPCE, :], PredY_unc[-13, idxCVLR, :]]).T
    fig, ax = bvar.plot_weighted_joint_and_marginals(YY, wStarMedian, 'PCE', 'U3', vis=True, LW=1.5, Y0=[PCE2026, U32026])
    ax.set_title('Forecast for Nov 2026 in Dec 2024')
    plt.savefig(f'{plotDir}/jointMarginal_PCE_LR_ET_202611.png', dpi=dots_per_inch)
    plt.close(fig)
    ...
    plt.xlabel('PCE inflation Rate', labelpad=15)  # Add padding to ensure the label is visible
    plt.ylabel('U3: Unemployment Rate')
    ...
    plt.savefig(f'{plotDir}/mean_matching_PCE_LR_ET_202711.png',bbox_inches='tight', dpi=dots_per_inch)
    plt.close()
    ...
    plt.title('Draws for Nov 2027 in Dec 2024 (weighted)')
    plt.grid(True)
    plt.box(True)
    plt.xlabel('PCE inflation Rate', labelpad=15)  # Add padding to ensure the label is visible
    plt.ylabel('U3: Unemployment Rate')
    ...
    plt.grid(color='lightgrey', linestyle='--', linewidth=0.5)
    plt.savefig(f'{plotDir}/draws_PCE_LR_ET_202711.png', bbox_inches='tight', dpi=dots_per_inch)
    plt.close()