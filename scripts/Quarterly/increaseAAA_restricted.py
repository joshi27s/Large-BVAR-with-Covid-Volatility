...
scenario_name = 'increaseAAA_restricted'
...
idxAAA = Spec.index[Spec['SeriesID'] == 'AAA'].tolist()[0]  # Moody's Seasoned AAA Corporate Bond Yield
# Index for macro variables and Treasury note yields
idxFixed = Spec.index[(Spec['isFinancial'] == 0) |
                      (Spec['SeriesID'].isin(['GS1', 'GS5', 'GS10']))].tolist()  # Macro vars and Treasury Yields

# Create a boolean array indicating variables used for plotting
idxCV = np.isin(range(n), idxFixed + [idxAAA])
# Create a matrix for shocks
Shock = np.nan * np.ones((h_fore.sum(), n))  # h_fore is a boolean array indicating forecasts
# Set shocks for the first forecast period
Shock[0, idxFixed] = 0  # Set macro variables to unconditional forecast (0 shock)
Shock[0, idxAAA] = 0.25  # 25 bp (basis points) increase from the baseline
