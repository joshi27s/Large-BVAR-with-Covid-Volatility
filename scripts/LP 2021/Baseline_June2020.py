...
T1estim = data.index[(data['DATE'].dt.year == 2020) & (data['DATE'].dt.month == 6)][0]    # end of estimation sample

T1av = T1estim       # date of last available data for forecasting
# date of last available data in the dataset
Tend = data.index[(data['DATE'].dt.year == 2021) & (data['DATE'].dt.month == 5)][0]
...
comparison_vintage_date = data.loc[T1estim, 'DATE'].strftime('%b%Y')  # Format as 'Feb_2020'
# Save all variables to a pickle file
with open(f'{results}/Baseline_{comparison_vintage_date}.pkl', 'wb') as f:
